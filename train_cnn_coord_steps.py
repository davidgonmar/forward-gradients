...
def make_dataset(split: str, batch_size: int, training: bool) -> Iterator[Tuple[jnp.ndarray, jnp.ndarray]]:
...
def main():
    ...
    def loss_and_acc_train(pflat, bstats, batch):
        ...
        logits, updates = model.apply(variables, x, training=True, mutable=["batch_stats"])
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for step in range(1, steps_per_epoch + 1):
            ...
            params_flat, batch_stats, rng, loss, acc = train_step(params_flat, batch_stats, rng, batch)
            bs = batch[0].shape[0]
        ...
        print(f"{epoch:5d} | {avg_train_loss:10.4f} | {avg_train_acc:9.3f} | {test_loss:9.4f} | {test_acc:8.3f} | {time.time()-t0:7.1f}")
    print("\nTraining complete.")
